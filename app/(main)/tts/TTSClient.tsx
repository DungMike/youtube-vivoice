...
import { ExternalLink, Loader2, Plus, Trash2, Volume2 } from 'lucide-react'
...
export default function TTSClient() {
  ...
  const handleConvertSingle = async (block: TextBlock) => {
    ...
    try {
      const response = await apiService.convertTextToSpeech(
        block.content,
        block.voice
      )
      if (response.success && response.data) {
  ...
  const handleConvertAll = async () => {
    const pendingBlocks = textBlocks.filter(
      block => block.status === 'pending' || block.status === 'error'
    )
    if (pendingBlocks.length === 0) {
      error(
        'No pending conversions',
        'All text blocks have already been converted'
      )
      return
    }
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        success(
          ...
          `${pendingBlocks.length} text blocks converted successfully`
        )
      } else {
        pendingBlocks.forEach(block => {
          updateTextBlock(block.id, {
        ...
        error(
          ...
          response.error || 'Failed to convert texts'
        )
      }
    } catch (err) {
      pendingBlocks.forEach(block => {
  ...
  const pendingCount = textBlocks.filter(
    block => block.status === 'pending' || block.status === 'error'
  ).length
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <Select
                        value={block.voice}
                        onValueChange={value =>
                          handleVoiceChange(block.id, value)
                        }
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">
              Add some text or upload files to get started with text-to-speech
              conversion
            </p>
          </CardContent>
        </Card>
