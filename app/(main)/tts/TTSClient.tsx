...
import { Plus, Trash2, Volume2, ExternalLink, Loader2 } from 'lucide-react'
...
import { textBlocksAtom, addTextBlockAtom, updateTextBlockAtom, removeTextBlockAtom, voicesAtom } from '@/lib/store'
import { useToast } from '@/hooks/useToast'
import { apiService } from '@/services/api'
import { TextBlock } from '@/types'

export default function TTSClient () {
  const [textBlocks] = useAtom(textBlocksAtom)
  ...
  const handleConvertSingle = async (block: TextBlock) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        updateTextBlock(block.id, { status: 'completed', audioUrl: response.data.audioUrl })
        success('Conversion complete', 'Your text has been converted to speech')
      } else {
        updateTextBlock(block.id, { status: 'error', error: response.error || 'Conversion failed' })
        error('Conversion failed', response.error || 'Failed to convert text to speech')
      }
    } catch (err) {
      updateTextBlock(block.id, { status: 'error', error: 'Network error' })
      error('Conversion failed', 'An unexpected error occurred')
    }
  }

  const handleConvertAll = async () => {
    const pendingBlocks = textBlocks.filter(block => block.status === 'pending' || block.status === 'error')
    ...
    try {
      const requests = pendingBlocks.map(block => ({ text: block.content, voiceId: block.voice }))
      const response = await apiService.convertMultipleTexts(requests)
      if (response.success && response.data) {
        response.data.forEach((result, index) => {
          const block = pendingBlocks[index]
          updateTextBlock(block.id, { status: 'completed', audioUrl: result.audioUrl })
        })
  ...
  const pendingCount = textBlocks.filter(block => block.status === 'pending' || block.status === 'error').length

  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold">Text-to-Speech</h1>
        <p className="text-muted-foreground">Convert your text content into natural-sounding speech</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Add Content</CardTitle>
          <CardDescription> Add text manually or upload text files to convert to speech </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="text" className="w-full">
            ...
            <TabsContent value="text" className="space-y-4 mt-6">
              <div className="space-y-2">
                ...
                <Textarea id="new-text" placeholder="Enter the text you want to convert to speech..." value={newText} onChange={(e) => setNewText(e.target.value)} rows={4} />
              </div>
              <Button onClick={handleAddTextBlock} disabled={!newText.trim()} className="w-full">
                <Plus className="mr-2 h-4 w-4" />
                Add Text Block
              </Button>
            </TabsContent>
            <TabsContent value="upload" className="mt-6">
              <FileUploader onFilesSelect={handleFilesUpload} acceptedTypes=".txt,.docx,.pdf" multiple={true} maxSize={5 * 1024 * 1024} />
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
        ...
        <>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleOpenVoiceClone} className="flex items-center gap-2">
                <ExternalLink className="h-4 w-4" />
                Clone New Voice
              </Button>
              <Button onClick={handleConvertAll} disabled={isConverting || pendingCount === 0} className="flex items-center gap-2">
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {block.fileName && (
                        <span className="text-sm font-medium text-muted-foreground">{block.fileName}</span>
                      )}
                    ...
                    <Button variant="ghost" size="icon" onClick={() => removeTextBlock(block.id)} className="h-8 w-8">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <Select value={block.voice} onValueChange={(value) => handleVoiceChange(block.id, value)} disabled={block.status === 'loading'}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    {(block.status === 'pending' || block.status === 'error') && (
                      <Button onClick={() => handleConvertSingle(block)} size="sm">
                  ...
                  {block.status === 'error' && block.error && (
                    <div className="text-sm text-red-600 dark:text-red-400">Error: {block.error}</div>
                  )}
                  {block.status === 'completed' && block.audioUrl && (
                    <AudioPlayer src={block.audioUrl} title={block.fileName || 'Generated Audio'} />
                  )}
                </CardContent>
              </Card>
            ))}
      ...
      {textBlocks.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">Add some text or upload files to get started with text-to-speech conversion</p>
          </CardContent>
        </Card>
      )}
    </div>
